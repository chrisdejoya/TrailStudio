...
export class ModelManager {
  ...
  addBoneHelper(skinnedMesh) {
    if (
      ...
      this.boneHelpers.some((h) => h.userData.skeleton === skinnedMesh.skeleton)
    )
      return;
  ...
  buildProceduralController() {
    ...
    const rockerMat = new THREE.MeshPhysicalMaterial({
      color: 0x222a36,
      roughness: 0.2,
      metalness: 0.6,
    });
    ...
    leftHandle.position.set(-1.7, -0.4, 0.6);
    leftHandle.rotation.z = 0.4;
    leftHandle.rotation.x = 0.2;
    leftHandle.castShadow = true;
    leftHandle.receiveShadow = true;
    proceduralGroup.add(leftHandle);

    const rightHandle = leftHandle.clone();
    rightHandle.position.set(1.7, -0.4, 0.6);
    ...
    this.register3DButton(
      0,
      makeMesh(
        new THREE.CylinderGeometry(0.12, 0.12, 0.1, 24),
        new THREE.Vector3(faceCenter.x, faceCenter.y, faceCenter.z + 0.28)
      )
    );
    this.register3DButton(
      1,
      makeMesh(
        new THREE.CylinderGeometry(0.12, 0.12, 0.1, 24),
        new THREE.Vector3(faceCenter.x + 0.28, faceCenter.y, faceCenter.z)
      )
    );
    this.register3DButton(
      2,
    ...
    this.register3DButton(
      4,
      makeMesh(new THREE.BoxGeometry(0.7, 0.18, 0.35), new THREE.Vector3(-1.1, 0.3, -0.95))
    );
    this.register3DButton(
      5,
      makeMesh(new THREE.BoxGeometry(0.7, 0.18, 0.35), new THREE.Vector3(1.1, 0.3, -0.95))
    ...
    this.register3DButton(
      6,
      makeMesh(new THREE.BoxGeometry(0.65, 0.3, 0.45), new THREE.Vector3(-1.1, 0.1, -1.3))
    );
    this.register3DButton(
      7,
      makeMesh(new THREE.BoxGeometry(0.65, 0.3, 0.45), new THREE.Vector3(1.1, 0.1, -1.3))
    ...
    this.register3DButton(
      ...
      makeMesh(
        ...
        new THREE.Vector3(-0.45, 0.33, -0.2)
      )
    );
    this.register3DButton(
      9,
    ...
    this.register3DButton(
      16,
      makeMesh(new THREE.CylinderGeometry(0.16, 0.16, 0.08, 24), new THREE.Vector3(0, 0.33, -0.2))
    );
    ...
    const createStick = (x, z) => {
      ...
      const stem = new THREE.Mesh(
        new THREE.CylinderGeometry(0.08, 0.08, 0.35, 16),
        new THREE.MeshPhysicalMaterial({ color: 0x505b6d, roughness: 0.5 })
      );
      stem.position.y = 0.15;
      stem.castShadow = true;
      group.add(stem);
      const cap = new THREE.Mesh(
        new THREE.CylinderGeometry(0.38, 0.32, 0.1, 32),
        new THREE.MeshPhysicalMaterial({ color: 0x62d8ff, roughness: 0.3, metalness: 0.6 })
      );
      cap.position.y = 0.32;
      cap.castShadow = true;
      group.add(cap);
    ...
    dpadCrossV.castShadow = true;
    dpadCrossH.castShadow = true;
    this.dpadRockerPivot.add(dpadCrossV);
    this.dpadRockerPivot.add(dpadCrossH);
    ...
    this.register3DButton(
      ...
      makeMesh(
        ...
        new THREE.Vector3(dpadBasePos.x, dpadBasePos.y, dpadBasePos.z - btnDist)
      )
    );
    this.register3DButton(
      13,
    ...
    this.register3DButton(
      ...
      makeMesh(
        new THREE.BoxGeometry(0.18, 0.08, 0.18),
        new THREE.Vector3(dpadBasePos.x - btnDist, dpadBasePos.y, dpadBasePos.z)
      )
    );
    this.register3DButton(
      15,
    ...
    if (node.isMesh || node.isBone || node.isGroup) {
      ...
      if (buttonIndex !== undefined) {
        ...
        const emissiveTargets = node.isBone
          ? this.findBoneEmissiveTargets(this.currentModel, node)
          : [node];
